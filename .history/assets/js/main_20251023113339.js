...
// Parallax scrolling effect - images rise from bottom
(function(){
  ...
  const updateParallax = () => {
    ...
    parallaxImages.forEach((img, index) => {
      ...
      // Images start below viewport and rise up past the top
      // At scrollProgress 0: image is below viewport
      // At scrollProgress 1: image has passed above viewport
      const startOffset = windowHeight + 200; // Start below viewport
      const endOffset = -600; // End above viewport
      const totalDistance = startOffset - endOffset;

      // Calculate position based on scroll progress and speed
      const yPosition = startOffset - (scrollProgress * totalDistance * speed);
      ...
      const baseOpacity = img.getAttribute('data-layer') === 'back' ? 0.7 : 1;
      img.style.opacity = opacity * baseOpacity;
    });
