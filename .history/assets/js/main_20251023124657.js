...
(function(){
  ...
  const updateParallax = () => {
    ...
    parallaxImages.forEach((img, index) => {
      ...
      // Group images in pairs/triplets so 2-3 images cross the text simultaneously
      const groupSize = 2.5; // How many images worth of time each image is visible
      const segmentSize = 1 / (totalImages * 0.6); // More overlap = more simultaneous images
      const imageStartProgress = index * segmentSize;
      const imageEndProgress = imageStartProgress + (segmentSize * groupSize);
