...
(function(){
  ...
  const parallaxContent = parallaxSection.querySelector('.parallax-hero__content');
  ...
  const updateParallax = () => {
    ...
    // Control text visibility - only show when section is in viewport
    if(parallaxContent){
      if(sectionTop <= windowHeight && sectionBottom >= 0){
        parallaxContent.style.opacity = '1';
        parallaxContent.style.visibility = 'visible';
      } else {
        parallaxContent.style.opacity = '0';
        parallaxContent.style.visibility = 'hidden';
      }
    }
    ...
    parallaxImages.forEach((img, index) => {
      ...
      // Each image gets a segment of the scroll journey
      // Image appears at its start point and disappears at its end point
      const segmentSize = 1 / (totalImages + 1); // Overlap segments slightly
      const imageStartProgress = index * segmentSize;
      const imageEndProgress = imageStartProgress + (segmentSize * 2.5); // Each image visible for 2.5 segments
      ...
      // Apply transforms
      img.style.transform = `translateY(${yPosition}px)`;
