...
# Evergreen Landscapes — static site starter

Test change: This line was added to verify repository updates.

A clean, modern, **HTML + CSS + JavaScript** template for a small landscaping business. It’s built to host on **GitHub Pages** with optional **custom domain**, and uses an external service (Formspree) to handle the contact form.
...
3. Install the “Live Server” extension (optional) to preview, or just open `index.html` in your browser.

> Tip: You already have great design instincts — all the copy, images, and colours are in one place: `assets/css/styles.css` and the four `.html` files.
...
**GUI (quickest):**
1. In VS Code, open the **Source Control** panel. Click **“Publish to GitHub”** and follow the prompts to create a new repo.  
...
gh repo create evergreen-site --public --source=. --remote=origin --push
```

**Docs:** Working with GitHub in VS Code and basic Git help are here:  
- VS Code × Git intro: https://code.visualstudio.com/docs/sourcecontrol/intro-to-git  
...
2. In your repository, go to **Settings → Pages → Custom domain** and enter your domain (e.g., `evergreen-gardens.co.uk`). This writes a `CNAME` entry for you.
...
> It can take up to a day for DNS to update. Make sure **“Enforce HTTPS”** is checked in the Pages section once it resolves.

Docs: GitHub’s custom domain guide.
...
> Alternatives: Netlify Forms, Basin, Getform, Formspark, Google Forms. On GitHub Pages you’ll need a third‑party form service or a serverless function.

---

## 6) Swap the images & copy quickly
- All images are hot‑linked from **Unsplash** and can be replaced with your own or other Unsplash URLs.
- Colours and spacing live in `assets/css/styles.css` under the `:root` CSS variables.
- Navigation labels and links are in each `.html` file. JS auto‑highlights the current page.

---

## 7) Costs (ballpark)
- **Hosting on GitHub Pages:** $0 (free)  
- **Domain:** typically **US$10–25/year** for common TLDs like .com, .co.uk or .nz. Exact price varies by registrar and TLD.  
...
- **Domain choice:** If the business is UK‑focused, prefer `.co.uk` or `.uk`. NZ‑focused: `.co.nz` or `.nz`. A `.com` works globally.
...
- **ChatGPT 5 Pro:** generate page copy, meta tags, alt text, and “voice” options. Paste your HTML and ask for improvements.  
- **GitHub Copilot (in VS Code):** inline suggestions and Copilot Chat for edits (“Convert this section to a 3‑column grid”, “Add dark‑mode”) and even small refactors.  
